...
- [ ] **#2 — Billboard sprite depth sorting: a melee attacker can render IN FRONT of a target that's actually closer to the camera.** Symptom (user, Q1 playtest): when a melee unit strikes a target nearer the camera, the attacker's billboard sometimes draws over the (closer) target. Almost certainly a depth-order issue in the billboarded-quad sprites, not the sim. Suspects: (a) transparent billboards commonly render with `depthWrite` off and rely on **draw order** for occlusion — if [SpriteRenderer](src/render/SpriteRenderer.ts)'s instanced draw order isn't sorted back-to-front by camera distance, two units at similar depth flip; (b) the **E6.A shove lunge** (`SHOVE_DISTANCE 0.35` toward the target, [BattleRenderer](src/render/BattleRenderer.ts)) pushes the attacker's quad toward the camera mid-strike, tipping the sort the wrong way exactly during an attack — which matches "happens when a melee attacks." Start: how SpriteRenderer sets per-instance depth / sort + the material's `depthTest`/`depthWrite`/`transparent` flags + the camera tilt; check whether disabling the shove makes it vanish (isolates cause b). Render-only; browser-verify (it's a visual-ordering bug, headless won't see it).

- [ ] **#3 — A kiting archer paths ON TOP of an enemy melee when trapped in a corridor (a real same-cell overlap).** **CONSISTENT repro (user, Q2 playtest, 2026-06-17):** an **archer sandwiched in a corridor between a FRIENDLY melee (behind) and a HOSTILE melee (in front)** *consistently* ends up on the hostile unit's cell. (First seen Q1 as a fuzzier "archer lerped into a merc" — this is the same bug, now reliably reproducible.) **Occupancy model confirmed:** units are **soft** blockers, NOT a hard one-unit-per-cell invariant — A* adds `occupiedCellPenalty` to route through an occupied cell, and a normal step onto an occupied forward cell triggers the E5.B perpendicular **sidestep** or an abstain ([movement.ts](src/sim/movement.ts) `buildMovementContext`/`stepToward`/`sidestep`). The corridor is the key: it removes the escape valves. Prime suspect now: the **O4 kite-out logic under a blocked retreat**. With `minRange` kiting the archer wants to step AWAY from the adjacent hostile melee, but the friendly melee occupies the only retreat cell and the corridor walls kill the perpendicular sidestep — so `nearestActingCell`'s `[minRange, range]` band selection (or the kite fallback) may resolve the *forward* (hostile-occupied) cell as the firing standoff, or A* routes through it since every detour is walled off (out-costs the finite penalty). Start: trace `nearestActingCell` + the O4 band predicate for a 1-wide corridor with both neighbors occupied — does it ever return the occupied forward cell? Add a determinism-safe assertion/test that no two units share a cell at a tick boundary, and a corridor kite-pin fixture (archer + friendly behind + hostile ahead, walls either side). Likely a real sim bug (not just the #2 visual), though #2 would amplify how it reads. Sim + render; reproduce headless first.
