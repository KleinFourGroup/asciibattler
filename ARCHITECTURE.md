...
turn:resolved           { turn; winner; reason; pool chips; result; pools }         # H4b: post-turn outcome cue (gated path only); 91a2: the chips are the APPLIED losses (not a re-derivation of the rule); 91d: reason (decisive | mutualWipe | cap) so the screen labels the chips by the rule set the turn paid
