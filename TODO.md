...
- [x] **Triage + report the JetBrains Mono `⊞` / `⊠` swap upstream** — ✅ 2026-09-18, triaged, NOTHING TO FILE: the raw vendored TTF has it (our pipeline ruled out); a DRAWING error (cmap + `post` names right, the outlines swapped); v2.304 is still the latest release; already reported upstream as JetBrains/JetBrainsMono#676 (OPEN since 2024-04, no fix PR; #697 its duplicate). `PRIMARY_EXCLUDES` stays until a release closing #676 is vendored — then re-run the swap search. One thing NOT checked: upstream `master`'s rebuilt TTF (2024-08-08) — a file download, not taken; a 2024-10 comment on #676 says still broken. Gotcha #136 · WORKLOG §101-triage.
