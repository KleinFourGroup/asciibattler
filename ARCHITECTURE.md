...
src/
  ...
  ui/
    ...
    ObjectiveController.ts   # J3/Q3: battle-scoped objective input — 78a left-click quick-Engage / right-click quick-Focus / arm(engage|focus)-then-click / hold / stop → World commands
...
turn:handRedrawn        { hand; drawPile; discardPile; grants; empowerMagnitudes }  # K3/49d: the pre-turn hand CHANGED (redraw, or 65c packet draw/discard — hand length may differ from the drawn size) — full new hand + the re-derived queue (K4: + re-derived badge column; R2: + refreshed draw/discard piles)
