/**
 * 72e/74e — the `--elite-chance` / `--port-chance` / `--event-chance`
 * node-scatter probe dials: parse-layer pin, mirroring drawAddArg.test.ts.
 * Run mode's [0, 1] probability guard owns rejecting explicit garbage
 * values at dispatch.
 */
...
describe('--elite-chance / --port-chance / --event-chance (72e/74e)', () => {
  it('parses numeric values and stays unset when absent', () => {
    ...
    expect(parseArgs(['--event-chance=1']).eventChance).toBe(1);
    expect(parseArgs(['--event-chance=0']).eventChance).toBe(0);
    expect(parseArgs(['--count=5']).eliteChance).toBeUndefined();
    expect(parseArgs(['--count=5']).portChance).toBeUndefined();
    expect(parseArgs(['--count=5']).eventChance).toBeUndefined();
    ...
    expect(parseArgs(['--event-chance']).eventChance).toBeUndefined();
  });
});
