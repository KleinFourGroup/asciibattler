...
- **NEXT — ROUND 7.5 (The Board), §109: the rule deletion + the round close — THE KICKOFF** (ROADMAP §109; `process/planning.md`: the code-reality audit, then a cut for the user to sign). §109 = spec D4 (the uniform quad-bottom anchor replaces R5–R7 + R10, with probes re-derived from the atlas and the special-rule count recorded before and after) and D8's last seams (the `anchor` dial, `restampAnchors`, the explorer trimmed to its keepers), then the three C1 doc drifts (TODO) and the round close. The spec: [round-7.5-spec.md](round-7.5-spec.md).
