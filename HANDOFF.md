...
- **NEXT — ROUND 7.5 (The Board): THE ROUND CLOSE, C1 (the macro re-audit, a `stop`), in a FRESH session** (the user's pre-commitment: over 350k of context means a new session; the §109 build session ended at 442k. ROADMAP §109's cut, signed 2026-09-26: C1 · C2–C5 one `stop` · C6; `process/planning.md` "Round close", `process/welfare-and-efficacy.md`; the Round 7 close, `archive/post-94-worklog.md` §"The Round 7 close", is the precedent). §109's build is done (109a–c, `0549789` · `72071bc` · `d94999b`): the quad-bottom anchor is the only rule, R5–R7 deleted, R10 re-grounded (the user's call; the rally X as a terrain mark is a TODO), 17 → 14 special rules, the explorer at its 11 keepers, the C1 drifts closed. The spec: [round-7.5-spec.md](round-7.5-spec.md).
  - The signed bookmark (106d) now means the defaults: the quad-bottom anchor is production (109a's pane oracle: identical to HEAD under `?bp=anchor-bottom`). The pre-7.5 camera is `bp=proj-persp_yaw-0`. Pane recipes: WORKLOG §106b, §106c-post, §106d step zero (the run driver) and §109a (the numeric oracle), and [process/browser-pane.md](process/browser-pane.md).
- **Constraints on 7.5.** DESIGN §UI idioms and §Input accessibility (signed at §103) are what Rounds 8 and 11 are checked against. DESIGN "Team identity on the board" holds the five clauses 7.5 must satisfy, answered by the ground mark as built (108e) and accepted by the user's grey read at stop 2. Yaw is not exposed to players (TODO, Round 11).
- **Trials this round** (revisit at the 7.5 close): the reads doctrine (AGENTS "Reads are cut, not improvised"). §105: 3 `batch` reads, all clear, no `-post` from them; 1 `-post` from the 105b stop. §106: 1 `batch` read (106b), clear; 2 `-post`s, one from each of the 106c and 106c-post stops; the 106d and 106e stops clear; 2 more `batch` reads (the 106d riders), clear. §107: 1 `batch` read (107b), clear; 1 `stop` (107d) with one `-post` from its finding (107d-post), whose `stop` is clear. §108: 0 `batch`, 2 `stop`s cut; stop 1 (the look, 108b) clear, no `-post`; stop 2 (108c–108e) clear, no `-post`. §109 build: 3 `none` (109a–c, each on an oracle or a test), 0 `batch`; one threshold call taken to the user mid-step (109a, the clip gate's controls). No open ◐. The AGENTS rewrite of 2026-09-23 (the tone audit, WORKLOG) is new too: watch whether sessions follow its "Before you… read…" triggers into `process/`.
- **Riders with dates:**
...
- **Last phase: §108 ✅ closed 2026-09-25.** The terrain draws the ground marks (a side shape per combatant, a plate under scenery, dashed on destructible walls and cover), on tops, step faces and mounds; the grey read passed; DESIGN holds the identity channel as built and the signed elevation clauses; upright depth ships without the hop; the marks cost +0.19 ms per frame in Firefox at 2560×1440, accepted; the mock and the hop dial are deleted. WORKLOG §108; §107 (the projection, built) before it.
