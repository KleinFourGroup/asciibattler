...
- **NEXT — ROUND 7.5 (The Board), §107: THE STOP at 107d, the user's played read — in progress: one finding (the diagonal-move clip), fixed as 107d-post (upright depth, the user's pick; ◐ built, read with 107d's stop). The read continues: 107d-post's diagonal past a shore corner, plus 107d's watches the user has not yet called clear.** The default view is ortho · pitch 45 · yaw 45 (`DEFAULT_CAMERA_VIEW`, 107d). 107a ✅, 107c ✅; 107b and 107d ◐ built, unread; 107b's `batch` read rides this stop. The read script is ROADMAP 107d's line; the verification is WORKLOG §107d. After the read: close §107, then the §108 kickoff. The cut is SIGNED (2026-09-24; ROADMAP §107, four steps 107a–d, one `stop` at 107d, the played read; the audit and a D8 deviation the user decided are in WORKLOG §107 Kickoff). The spec: [round-7.5-spec.md](round-7.5-spec.md).
...
- **Trials this round** (revisit at the 7.5 close): the reads doctrine (AGENTS "Reads are cut, not improvised"). §105: 3 `batch` reads, all clear, no `-post` from them; 1 `-post` from the 105b stop. §106: 1 `batch` read (106b), clear; 2 `-post`s, one from each of the 106c and 106c-post stops; the 106d and 106e stops clear; 2 more `batch` reads (the 106d riders), clear. §107: open ◐ 107b (`batch` → 107d), 107d and 107d-post (`stop`). The AGENTS rewrite of 2026-09-23 (the tone audit, WORKLOG) is new too: watch whether sessions follow its "Before you… read…" triggers into `process/`.
- **Riders with dates:**
...
- **Last phase: §106 ✅ closed 2026-09-23.** The projection spike, pass two: the merged ground mark, the scenery plate, marks cut to the tiles and draped, N×N slabs centred, the anchor rule deletable, the bookmark signed after a full run, and the round's spec signed. §105 (pass one, 2026-09-22) named ortho · pitch 45 · yaw 45 with the lens-20 fallback. WORKLOG §105 and §106.
