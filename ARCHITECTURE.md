...
command:applied         { tick; command: WorldCommand }                             # 53a: a drained WorldCommand took effect, stamped with its apply tick (a parked drain stamps the frozen tick); the trace recorder's substrate — fires ONLY for real drained commands (an auto-revert emits objective:cleared, never this)
