...
turn:starting           { turn; hop; pools; hand; drawPile; discardPile; grants; empowerStacks; daemons; map; poolAtRisk }  # H4b/H5b/K3/K3.5/K4/L1/R2/49d/89e: pre-turn gate cue (gated only); poolAtRisk = the fielded wave's chip bound, previewed off the keyed battle stream (display-only); hand + the other two piles (R2, recruitment order) + the GRANT QUEUE (TurnGrantView[] — per-source redraw+empower in acquisition order, `active` = the cursor) + per-card empower stacks + the OWNED daemons [{id;name;description;redrawGate;empowerGate}] + the ENCOUNTER's map
turn:resolved           { turn; winner; pool chips; result; pools }                 # H4b: post-turn outcome cue (gated path only)
pools:chipped           { turn; playerBefore; playerAfter; enemyBefore; enemyAfter } # 89a: the pools as resolveTurn APPLIED them (both paths) — the fuzz telemetry's trajectory source; after−before = the chip rule's charge, rule-agnostic
