...
Two issues the user surfaced during the **Q1** playtest (2026-06-17) — pause + 0.5× made them visible. Deferred from the Q HUD overhaul by user agreement; grouped here as "Phase Qb." Neither blocks Q. (The third Q1 finding — the action progress bar ignoring game speed — was a UI bug fixed immediately, not deferred.)
...
- [ ] **#3 — A kiting archer appeared to lerp INTO a pursuing mercenary (possible transient same-cell overlap).** Symptom (user, Q1 playtest, lower confidence): during O4 kiting (archer backs off, merc chases), the archer's sprite looked like it slid through/into the merc. **Occupancy model confirmed (2026-06-17):** units are **soft** blockers, NOT a hard one-unit-per-cell invariant — A* adds `occupiedCellPenalty` to route through an occupied cell and a normal step onto an occupied forward cell triggers the E5.B perpendicular **sidestep** or an abstain ([movement.ts](src/sim/movement.ts) `buildMovementContext`/`stepToward`/`sidestep`). So units *generally* don't share a cell, but nothing forbids a **same-tick** coincidence. Prime suspect: simultaneous move resolution — `buildMovementContext` reads each unit's **pre-tick** position, so if two units resolve moves in the same tick whose committed cells coincide (or A* routes one *through* the other when the detour out-costs the penalty), a 1-tick overlap is possible; the `unit:moved` lerp then animates the slide and **#2's depth bug makes the overlap read as interpenetration**. Could also be purely visual (the lerp window vs step cadence) with no real overlap. Start: check `World.tick`'s move-resolution order — sequential-with-live-occupancy vs batch-against-pre-tick — and whether a commit re-validates the destination; consider a determinism-safe assertion/test that no two units share a cell at a tick boundary. Sim + render; reproduce headless first (drive a kite-vs-pursuit fixture, assert occupancy each tick) before the browser.
