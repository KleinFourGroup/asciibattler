...
- **NEXT — ROUND 7.5 (The Board), §106: the projection spike, pass two + the spec.** Kicked off 2026-09-22; the cut and its reads are user-signed (ROADMAP §106, WORKLOG §106 Kickoff).
  - Done: 106a the instrument · 106b the `slab` dial (read clear) · 106c `ground` + `plate` (read: `merged` wins; the anchor rule R5–R7 + R10 is deletable; the plate is `filled`) · 106c-post marks cut per tile onto the tile tops, `plateScope`, `plateAlpha` (read clear) · 106c-post2 the step-face drape (read clear: on; `cueDepth` world; `plateAlpha` 0.6 kept).
  - **(1) 106d, the played read** (`stop`, open: step zero CLEAR 2026-09-23 — a whole run, a `resetRun` and a boss run played under the bookmark with 0 audit findings, WORKLOG §106d). The bookmark: `?bp=proj-ortho_yaw-45_cue-outline_cueAlpha-0.3_slab-centre_ground-merged_plate-filled_plateScope-all_anchor-bottom_drape-1_cueDepth-world` (`anchor-bottom`: the user's yes, so the rule deletion gets played). Watch the overlay stack in clumps (three fixes in reserve, WORKLOG §THE VERDICT finding 3), the wall staircase, and picks under yaw.
  - **(2) 106e, the spec** (`stop`). It owns the spike code's disposition, the scroll-mode pan under yaw, the terrain-decal question, the elevation clauses, and the rule deletions (the table: WORKLOG §Kickoff).
