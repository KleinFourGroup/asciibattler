...
**B1 landed (palette direction).** Side-by-side decision-point demo of 4 variants (strict / hue-locked / sat-clamped / hue-locked+bloom). User picked: drop palette-quant entirely (palette becomes art-direction discipline, not shader enforcement), keep saturation-clamp + bloom. Main composer chain is now `RenderPass → SatClamped → Bloom → Scanlines → OutputPass`. UnrealBloomPass's high-pass is patched to use `max(R,G,B)` instead of Rec.709 luminance so NEON_RED enemies glow on equal footing with TERMINAL_GREEN allies. SpriteRenderer gained a per-instance `bloomIntensity` attribute (default 1.0) for forced-glow effects — currently unused; B3/C2 will reach for it. Gotchas #1, #3, #4 retired as a consequence. Next up per ROADMAP:

1. **B3 — Floating per-unit HP bars + action progress bar.** Big perceptual win; builds on A1's `activeAction` duration as the progress-bar source. Bar geometry (billboarded world-space quad with shader-cutoff fill, instanced via the SpriteRenderer pattern). Fill color can use `bloomIntensity` for full-HP glow.
2. **B6 — Audio.** Pending audio asset selection.
3. **C2 — New archetypes (mage, rogue, healer).** Unblocked by A1+A2; A4 makes adding their stat tables a `config/archetypes.json` edit. Mage charge-up is a natural `bloomIntensity`-ramp use case.
...
30. **B1 per-sprite `bloomIntensity` is an instance attribute, not a uniform.** Lives on `SpriteRenderer` alongside `instancePosition` / `instanceColor` / `instanceAlpha`; the sprite vertex shader passes it through, fragment shader multiplies output color by it. Default 1.0. Bumping it past 1.0 pushes the output past the bloom threshold for forced glow. Don't conflate with `alpha` — alpha modulates coverage; bloomIntensity modulates color magnitude (and bleeds into adjacent pixels via the bloom blur).
