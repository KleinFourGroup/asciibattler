...
//
// Per-vertex inputs (in addition to the standard `position` + `normal`):
//   aColor  — vertex color baked from tile height + face shading.
//   aTopUV  — (0..1, 0..1) on the top face, (0,0) on side faces.
//
// `vIsTop` is set from the model-space normal so the fragment can draw
