...
- **NEXT — ROUND 7.5 (The Board), §106: the projection spike, pass two + the spec.** Kicked off 2026-09-22; the cut and its reads are user-signed (ROADMAP §106, WORKLOG §106 Kickoff).
  ...
  - **(1) 106d, the played read** (`stop`). Step zero: a whole run plays under the bookmark `?bp=proj-ortho_yaw-45_cue-outline_cueAlpha-0.3_slab-centre_ground-merged_plate-filled_plateScope-all_anchor-bottom_drape-1_cueDepth-world` (`anchor-bottom`: the user's yes, so the rule deletion gets played). Watch the overlay stack in clumps (three fixes in reserve, WORKLOG §THE VERDICT finding 3), the wall staircase, and picks under yaw.
