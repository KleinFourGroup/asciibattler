# Retro scratchpad — Cluster 5 (Map Content, Phases 73→83, 2026-08-05 → 2026-08-21)

Archived 2026-08-21 at the §83g cluster close (the distillation ritual's
sixth run). Promoted before archiving: the circular-verification,
distribution-surface, forgetful-path-guard, add-a-consumer, per-ID
config-surgery, queue-file-driver/one-HEAD-per-cohort, and
typechecked-probes norms → AGENTS; the undisplayed-pane synchronous
drive, the stale-HMR-tab, and the default-seed-bias tips → HANDOFF
browser-verify tips; the fixed-hand-probe, aggregate-identity,
per-site-null-semantics, and cross-HEAD-pooling caveats → BALANCE
§Caveats; the scan-don't-pin canary, derive-don't-re-pin, and
gate-first-TDD patterns → TESTING. Kept here as-is: the Bash `\$`
papercut, the tail-seed note, the shared-body note, the "fake
counterplay" reframe, the mid-phase counter-design note, and the
cross-model author/auditor EXPERIMENT card (an experiment, not a norm —
per its own text). Entries below verbatim.
