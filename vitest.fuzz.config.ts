 ...
 * after §36b's deferred moves + pursuit-hold lengthened battles, and again
 * after §45's cooperation changes shifted greedy-run TRAJECTORIES (several
 * seeds run materially deeper/longer — sim content, not compute: the §45c
 * perf probe measured per-tick cost +~1% on equal seeds; 30s → 90s, the 43a
 * precedent). A genuine sim deadlock is caught by the harness tick-cap
 * (resolve-as-draw), NOT this wall-clock guard, so a generous default can't
 * mask a hang. The heaviest corpus sweeps (occupancyInvariant) still set
 * their own higher per-test timeouts.
 */
