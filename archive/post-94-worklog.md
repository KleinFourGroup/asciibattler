...
### C5 — the archive + the cursor: ROUND 7 (IDIOMS) CLOSES, 2026-09-20

The pair + the spec archived as `archive/post-94-roadmap.md` +
`archive/post-94-worklog.md` (THIS file) + `archive/round-7-spec.md` — the
name is the `post-88-*` precedent applied ("post-" + the previous round's
last phase), confirmed by the user at the cut. Fresh `ROADMAP.md` +
`WORKLOG.md` stubs open Round 7.5 (The Board) at §105, holding the charter
only: the projection spike comes first, the spec after it, the cut — WITH
READS — at the kickoff. The closed-rounds paragraph is appended to
`archive/closed-rounds.md`, its one line to HANDOFF; META-ROADMAP marks
Round 7 closed; the HANDOFF cursor is rewritten (Round 7's two verbose
phase rows demoted to one-liners; two dated riders added — the nudge-reword
check, due 2026-09-27 or the next round close, and the recorded model id).

**The close, in one breath.** Five commits, one session across two days:
C1 the macro re-audit RE-CHARTERED Round 7.5 (the user contested the
premise; the spike-first shape, the elevation requirement for Round 9's
flyer, no renumber) · C2 the first efficacy read + `npm run friction-scan`
· C3 the first welfare read — four standing decisions, the reworded
reminder on trial, and "pause after every commit" replaced by "reads are
cut, not improvised" (on trial through 7.5) · C4 the ninth sweep (ten
AGENTS amendments, gotchas #137–#138, the docs-cap off-by-one) · C5 this.
Tests 3006 throughout; no src change outside two `scripts/` instruments
and the docs guard's ruler; no smoke, no bump. Commits `a4a2d67` →
`accd1fe` → `315a98e` → `07a6344` → the close commit.
