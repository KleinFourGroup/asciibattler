...
- [ ] **Renderer capacity audit (post-C1b).** All three instanced renderers are fixed-size: `SpriteRenderer` and `BarRenderer` cap at 256, `WaterRenderer` at 144 (= 12² exactly). Today's load on a 12×12 battle is ~30 sprites + ~20 bars + ~6 water tiles — comfortable headroom — but the caps will start to bite as the arena grows (C1c grid changes, dense custom layouts from C1d's editor, larger unit counts in C6). Two specific fixes:
  - **SpriteRenderer / BarRenderer** throw `capacity exhausted` on overflow. Quick fix: pass a larger capacity from `Game.ts` (`new SpriteRenderer(this.fontAtlas, 1, 1024)`). Durable fix: auto-grow on `addSprite` — allocate a bigger `Float32Array`, copy, re-upload. ~30 lines per renderer.
  - **WaterRenderer** silently `break`s past 144 — water tiles past slot 144 just don't render, no error. Should be coupled to `gridSize²` explicitly at construction. The silent truncation is the worse bug; throw or scale, don't drop.
  
  Crosses ~256 sprites at roughly a 32×32 grid with 25% wall density, or a 16×16 grid that's mostly walls. Neither is on the immediate horizon, but worth fixing before grid-size becomes tunable rather than fixed at boot.
