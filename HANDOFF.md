...
- **NEXT — ROUND 7.5 (The Board), §108: the ground mark, drawn by the terrain — IN PROGRESS, the cut signed 2026-09-24** (ROADMAP §108: six steps, two stops; the audit's 16 findings and the six decisions in WORKLOG §108 Kickoff). 108a ✅, 108b ✅ (stop 1 read clear 2026-09-25, the defaults signed), 108c ✅, 108d ✅, 108e ✅ (STOP 2 read clear 2026-09-25: the grey read passed, upright depth alone ships, Firefox +0.19 ms per frame acceptable, the wording signed; WORKLOG §108 STOP 2), 108f ✅ (the mock seams, the look dials but `plateCorner`, and the hop dial deleted; the marks' pixels byte-identical across it). **Next: the §108 close**, then the §109 kickoff. §108 = spec D3 (SDF marks in the terrain shader from a per-tile table), D5 (identity; the grey read at stop 2) and D6 (the elevation clauses). The spec: [round-7.5-spec.md](round-7.5-spec.md).
  - The signed bookmark (106d) is `?bp=anchor-bottom` since 108f: the rest of it named the mock's dials, and its long form still parses to that (`state.test.ts`). The pre-7.5 camera is `bp=proj-persp_yaw-0`. Pane recipes: WORKLOG §106b, §106c-post and §106d step zero (the run driver), and [process/browser-pane.md](process/browser-pane.md).
...
- **Tests:** **3121 main (0 skipped) + 582 fuzz:smoke**, green; main re-counted 2026-09-25 (108f, the hook before its commit: 17 tests left with the mock and the hop), fuzz:smoke last run at `a9c16ef`. Typecheck clean. The hook takes ~45 s for docs and UI, ~7 min when the smoke runs.
