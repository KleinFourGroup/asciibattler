...
- [ ] **The AoE-status enemies (banshee et al.) — the PRESENTATION half** (verdict 7a, split at the Round 7 spec 2026-09-09): `wail` / `hex` are impact-only fx with no release / travel phase — no projectile exists anywhere in `abilities.json` (WORKLOG §Kickoff H.9). A `release` phase + a projectile fx key → **§102**.
...
- [ ] **A run-end stats screen** (the 94d ledger's purpose beyond the post-turn rows — the user's "lots of good stats once a run ends" for the roguelike audience): `Run.fallenLedger` (v45) is the source — per encounter / per side / per archetype; the GameOverScreen and the victory screen are the surfaces. A Round 7 polish item.
...
- [ ] **Triage + report the JetBrains Mono `⊞` / `⊠` swap upstream** (gotcha #136; the user's call, 2026-09-18). What is PROVEN: our generated subset woff2 — built by `subset-font` (harfbuzz) from the vendored `JetBrainsMono-Regular.ttf`, which is the upstream GitHub release **v2.304** zip (sha256 in `assets/fonts/jetbrains-mono/README.md`; NOT an npm font package — fontsource was dropped at §79g) — paints U+229E as a boxed X and U+22A0 as a boxed plus, in both the Chromium pane and the user's Firefox. What is NOT yet proven, and is the triage: (1) **render from the RAW vendored TTF**, not the subset (a scratch page with an `@font-face` on the `.ttf`, or dump the two glyph names / outlines from `glyf` + `post`) — a harfbuzz subsetter swapping two glyphs is very unlikely, but one check rules our pipeline out; (2) check whether it is a **cmap** error (the codepoints point at each other's glyphs — glyph names `uni229E` / `uni22A0` or `squaredplus` / `squaredtimes` will say) or a **drawing** error (the right names on the wrong outlines) — the report should say which; (3) check the **latest upstream release and `master`** (<https://github.com/JetBrains/JetBrainsMono>) — if fixed there, the action is a vendored-font upgrade, not a report, and the upgrade re-runs the swap search; (4) **search upstream issues** for U+229E / U+22A0 / "squared plus" before filing. If it survives all four: file at the upstream tracker with the two codepoints, the release, a side-by-side against another face, and which table is wrong. Our `PRIMARY_EXCLUDES` stays either way until a fixed release is vendored — then it becomes a harmless no-op and can be emptied after a swap-search re-run.
