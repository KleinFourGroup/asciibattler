...
- **NEXT — ROUND 7.5 (The Board), §107: the projection, built — 107d (the flip, then THE STOP).** 107a ✅, 107c ✅; 107b ◐ built, its `batch` read rides 107d's stop. The cut is SIGNED (2026-09-24; ROADMAP §107, four steps 107a–d, one `stop` at 107d, the played read; the audit and a D8 deviation the user decided are in WORKLOG §107 Kickoff). The spec: [round-7.5-spec.md](round-7.5-spec.md).
  - The signed bookmark (106d), for any comparison in play: `?bp=proj-ortho_yaw-45_cue-outline_cueAlpha-0.3_ground-merged_plate-filled_plateScope-all_anchor-bottom_drape-1_cueDepth-world`. Pane recipes: WORKLOG §106b, §106c-post and §106d step zero (the run driver), and [process/browser-pane.md](process/browser-pane.md).
...
- **Trials this round** (revisit at the 7.5 close): the reads doctrine (AGENTS "Reads are cut, not improvised"). §105: 3 `batch` reads, all clear, no `-post` from them; 1 `-post` from the 105b stop. §106: 1 `batch` read (106b), clear; 2 `-post`s, one from each of the 106c and 106c-post stops; the 106d and 106e stops clear; 2 more `batch` reads (the 106d riders), clear. §107: open ◐ 107b (`batch` → 107d). The AGENTS rewrite of 2026-09-23 (the tone audit, WORKLOG) is new too: watch whether sessions follow its "Before you… read…" triggers into `process/`.
- **Riders with dates:**
