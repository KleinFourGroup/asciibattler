...
- **NEXT — ROUND 7.5 (The Board), §108: the ground mark, drawn by the terrain — its KICKOFF, best in a fresh session** (risk medium-high, so a planning session of its own per [process/planning.md](process/planning.md)). §108 = spec D3 (the marks as signed-distance shapes in the terrain shader, from a per-frame table binned per tile), D5 (team identity, with the grey read as the acceptance test) and D6 (the elevation clauses into DESIGN). Its decision points (ROADMAP §108) now include 107d-post's re-read: upright depth against the hop, once the marks exist (`tests/board/clip.ts` measures both). Inputs for the audit: the spike's `GroundCues` and `conform.ts` (what D8 deletes there); D3's frame-cost measurement needs a before, taken without marks at the user's resolution; a unit sprite's position is still its ground point (107d-post changed only depth), while a slab's is slid and its marks read `footprintCentre`. The spec: [round-7.5-spec.md](round-7.5-spec.md).
...
- **Tests:** **3107 main (0 skipped) + 582 fuzz:smoke**, green; main re-counted 2026-09-24 (107d-post), fuzz:smoke last run at `a9c16ef`. Typecheck clean. The hook takes ~45 s for docs and UI, ~7 min when the smoke runs.
