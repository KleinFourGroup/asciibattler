...
1. **D4 — Camera overhaul (dev fit + game scroll).** Now that arenas can be up to 32×32, the single fixed-fit camera shows the player 36% of a 20-wide procedural and ~14% of a 32-wide hand-authored. Roadmap: keep the current fit as Mode A (dev default), add Mode B (scroll camera with WASD + RTS-style edge scroll). Default-mode question deferred to beta-testing; recommendation is to flip to scroll once D5 spawn regions land so the initial camera focus has a sensible anchor. Decision points: scroll-window size (recommend 12×12 for continuity with pre-D3 framing), edge-scroll thresholds, default-mode timing.
