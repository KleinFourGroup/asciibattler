...
- **NEXT — ROUND 7.5 (The Board), §107: the projection, built — its KICKOFF, in a fresh session.** The spec is SIGNED ([round-7.5-spec.md](round-7.5-spec.md), 2026-09-23); §107–§109 are entered in ROADMAP with charters, exits and scope guards, no sub-steps. §107 = spec D1 (ortho · pitch 45 · yaw 45 becomes `DEFAULT_CAMERA_VIEW`), D2 (the N×N slab rule replaces R11) and D7's dev pan. Kickoff per [process/planning.md](process/planning.md): audit the camera code as it is now (the spec's D1 lists what is owed; the 105d seam already carries both cameras), draft the cut with its reads, shape-lock with the user.
  - **Open ◐ (`batch`, due at §107's first stop): the two 106d-rider bugs**, fixed 2026-09-23 and not yet read: hills follow the theme (`c348964`); the empower chips fit the compact card (`0970be0`). Read scripts: WORKLOG §"The 106d riders".
  - The signed bookmark (106d), for any comparison in play: `?bp=proj-ortho_yaw-45_cue-outline_cueAlpha-0.3_slab-centre_ground-merged_plate-filled_plateScope-all_anchor-bottom_drape-1_cueDepth-world`. Pane recipes: WORKLOG §106b, §106c-post and §106d step zero (the run driver), and [process/browser-pane.md](process/browser-pane.md).
...
- **Trials this round** (revisit at the 7.5 close): the reads doctrine (AGENTS "Reads are cut, not improvised"). §105: 3 `batch` reads, all clear, no `-post` from them; 1 `-post` from the 105b stop. §106: 1 `batch` read (106b), clear; 2 `-post`s, one from each of the 106c and 106c-post stops; the 106d and 106e stops clear; 2 `batch` reads open (the 106d riders). The AGENTS rewrite of 2026-09-23 (the tone audit, WORKLOG) is new too: watch whether sessions follow its "Before you… read…" triggers into `process/`.
- **Riders with dates:**
...
- **Tests:** **3093 main (0 skipped) + 582 fuzz:smoke**, green; main re-counted 2026-09-23 (the 106d riders), fuzz:smoke last run at `a9c16ef`. Typecheck clean. The hook takes ~45 s for docs and UI, ~7 min when the smoke runs.
