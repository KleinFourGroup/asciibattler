# [ARCHIVED] Balance-sweep protocol + log (H7c → Phase O)

> **Superseded 2026-06-21.** This is the historical record of the **global-knob**
> balance era (H7c through Phase O — the sweeps that derived the `1.25 × 1.5` band
> by mutating the live `DIFFICULTY` object). Phase V replaced the random wave
> generator with authored encounters, so the lever + the unit of analysis changed;
> the live protocol is now [BALANCE.md](../BALANCE.md) (per-encounter). Kept for the
> run-log history + the methodology bones BALANCE.md carries forward. Nothing here
> is the current procedure — read BALANCE.md for that.
