# Archived retro scratchpad — the micro round (53→60)

Swept 2026-07-21 at the Cluster-4 kickoff (the distillation ritual's third
run). Disposition of each entry:

- **57g — background batches survive a harness crash** → PROMOTED to AGENTS
  ("How we collaborate": the long-batch discipline bullet — timeout/artifact
  verification + the orphan-check recovery protocol).
- **§54a — the 45-minute test hang** → PROMOTED three ways: the bounded-steps
  /never-poll-`currentTick` pattern to TESTING.md ("Adding a test"); the
  background-batch-timeout half merged into the AGENTS bullet above; the
  pre-commit fuzz-trigger gap FIXED in [.githooks/pre-commit](../.githooks/pre-commit)
  (`src/bot/` + `tests/fuzz/` added to the trigger regex — manual fuzz:smoke
  on harness commits is no longer needed).
- **§54d–g — spot-check discipline** → PROMOTED to BALANCE.md Caveats
  (full-board-before-commit rule; mine-the-trace-data-first).
- **§56 close** → PROMOTED: the interior-boundary window-testing pattern to
  TESTING.md; the cell-volatility / probes-arbitrate-cells-attribute doctrine
  to BALANCE.md Caveats. (The reconciliation-sweep instrument was already
  homed in TODO.md.)
- **ROADMAP line guards vs a mid-round expansion** → RESOLVED: the
  closed-phase demotion rule was ADOPTED at the §60f close (user-signed,
  `7aeafae`) and now lives in AGENTS "Legal ROADMAP mutations"; the
  docs.test.ts caps re-size at each round's roadmap authoring.
