...
- [ ] **Rename the `draw-two` packet id (it's Surge, and it draws THREE).** The id was authored at §65 when the packet drew 2; 72f buffed the draw to 3 (display name "Surge") and DELIBERATELY kept the id — it's a serialized key (RunSnapshot cache entries carry raw packet ids). The rename is trivial config+grep work (`config/packets.json` + any fuzz `--grant` fixtures/scripts referencing `draw-two`; decisions.csv item keys are outputs, no action) but it invalidates dev-saves carrying the old id, so **land it inside the §74 v40→v41 bump window** where stale saves reject wholesale anyway. Surfaced 2026-08-05 at the 73c native eyeball (user call).
