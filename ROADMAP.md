...
- [ ] **108f** — the mock seams deleted (D8) with the read dials, except `plateCorner` (kept for organic scenery, the user's call at stop 1); the hop dial and its code deleted (the stop-2 verdict; `board-wade` stays); the bench's GPU name read from `RENDERER` first (the Firefox warning, WORKLOG §108 STOP 2); the bookmark → `anchor-bottom`. Read `none` (the marks' pixels unchanged across the deletion).
