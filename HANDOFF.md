...
- **NEXT — ROUND 7.5 (The Board), §108: the ground mark, drawn by the terrain — IN PROGRESS, the cut signed 2026-09-24** (ROADMAP §108: six steps, two stops; the audit's 16 findings and the six decisions in WORKLOG §108 Kickoff). 108a ✅, 108b ✅ (stop 1 read clear 2026-09-25, the defaults signed), 108c ◐ (the hop dial), 108d ◐ (the frame-cost bench) and 108e ◐ (the DESIGN text), built 2026-09-25. **Next: STOP 2, open** (the user's reads, scripted in WORKLOG §108e), then 108f (the grey read in a live battle, the posed flyer at lift 0.45, upright vs the hop, the user's Firefox bench run, the DESIGN wording). §108 = spec D3 (SDF marks in the terrain shader from a per-tile table), D5 (identity; the grey read at stop 2) and D6 (the elevation clauses). The mock (`GroundCues`, `conform.ts`) stays until 108f: it is the known answer for "production matches the signed look". The spec: [round-7.5-spec.md](round-7.5-spec.md).
  - The signed bookmark (106d), for any comparison in play, shortened now that its projection and slab rule are production: `?bp=cue-outline_cueAlpha-0.3_ground-merged_plate-filled_plateScope-all_anchor-bottom_drape-1_cueDepth-world` (the long form still parses). The pre-7.5 camera is `bp=proj-persp_yaw-0`. Pane recipes: WORKLOG §106b, §106c-post and §106d step zero (the run driver), and [process/browser-pane.md](process/browser-pane.md).
- **Constraints on 7.5.** DESIGN §UI idioms and §Input accessibility (signed at §103) are what Rounds 8 and 11 are checked against. DESIGN "Team identity on the board" holds the five clauses 7.5 must satisfy, answered by the ground mark as built (108e); the grey read that accepts them is stop 2's. Yaw is not exposed to players (TODO, Round 11).
- **Trials this round** (revisit at the 7.5 close): the reads doctrine (AGENTS "Reads are cut, not improvised"). §105: 3 `batch` reads, all clear, no `-post` from them; 1 `-post` from the 105b stop. §106: 1 `batch` read (106b), clear; 2 `-post`s, one from each of the 106c and 106c-post stops; the 106d and 106e stops clear; 2 more `batch` reads (the 106d riders), clear. §107: 1 `batch` read (107b), clear; 1 `stop` (107d) with one `-post` from its finding (107d-post), whose `stop` is clear. §108: 0 `batch`, 2 `stop`s cut; stop 1 (the look, 108b) clear, no `-post`. Open ◐: 108c, 108d, 108e (read at stop 2). The AGENTS rewrite of 2026-09-23 (the tone audit, WORKLOG) is new too: watch whether sessions follow its "Before you… read…" triggers into `process/`.
- **Riders with dates:**
...
- **Last phase: §107 ✅ closed 2026-09-24.** The projection, built: ortho · pitch 45 · yaw 45 is `DEFAULT_CAMERA_VIEW`, the N×N slab rule is production, the dev pan follows the yaw, and standing sprites are depth-tested upright (107d-post, fixing the diagonal-move clip found in the played read, pre-existing). WORKLOG §107; §106 (the spike, pass two) and §105 before it.
...
- **Tests:** **3138 main (0 skipped) + 582 fuzz:smoke**, green; main re-counted 2026-09-25 (108d, `npm test` before its commit), fuzz:smoke last run at `a9c16ef`. Typecheck clean. The hook takes ~45 s for docs and UI, ~7 min when the smoke runs.
