...
1. **D3 — Variable map sizes.** Procedural rolls a side length uniformly in [10, 20] (square); hand-authored layouts declare their own `gridSize` (8-32). Retires `config/terrain.json`'s `spawnRowsClear` field (D5 replaces row reservation with explicit spawn regions). Existing four layouts retrofit with `gridSize: 12`. `Renderer.fitCamera()` becomes per-battle-size-aware. **Decision point at the start:** rectangular layouts (`gridW`/`gridH`) for hand-authored, or square-only across the board? Roadmap recommends rectangular for hand-authored, procedural stays square. Editor scope in this step: grid-size selector (dropdown).
