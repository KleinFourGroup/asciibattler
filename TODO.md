...
- [ ] **Renderer capacity audit.** `SpriteRenderer` and `BarRenderer` are fixed-size at 256 instances and throw `capacity exhausted` on overflow. Today's load on a 12×12 battle is ~30 sprites + ~20 bars — comfortable headroom — but the caps will start to bite as the arena grows (dense custom layouts from C1d's editor, larger unit counts in C6). Quick fix: pass a larger capacity from `Game.ts` (`new SpriteRenderer(this.fontAtlas, 1, 1024)`). Durable fix: auto-grow on `addSprite` — allocate a bigger `Float32Array`, copy, re-upload. ~30 lines per renderer. Crosses 256 sprites at roughly a 32×32 grid with 25% wall density, or a 16×16 grid that's mostly walls. (The C1a `WaterRenderer` cap that originally lived in this item is no longer relevant — C1c folded water rendering into `TerrainRenderer`, which sizes its buffers from `gridSize²` at construction.)
